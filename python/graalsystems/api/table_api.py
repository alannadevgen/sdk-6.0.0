"""
    Tenant API

    Tenant API  # noqa: E501

    The version of the OpenAPI document: 0.0.1
    Contact: abc@layer.fr
    Generated by: https://openapi-generator.tech
"""


import re  # noqa: F401
import sys  # noqa: F401

from graalsystems.api_client import ApiClient, Endpoint as _Endpoint
from graalsystems.model_utils import (  # noqa: F401
    check_allowed_values,
    check_validations,
    date,
    datetime,
    file_type,
    none_type,
    validate_and_convert_types
)
from graalsystems.model.error import Error
from graalsystems.model.patch import Patch
from graalsystems.model.table1 import Table1


class TableApi(object):
    """NOTE: This class is auto generated by OpenAPI Generator
    Ref: https://openapi-generator.tech

    Do not edit the class manually.
    """

    def __init__(self, api_client=None):
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client
        self.create_table_endpoint = _Endpoint(
            settings={
                'response_type': (Table1,),
                'auth': [
                    'internal'
                ],
                'endpoint_path': '/layers/{layer_name}/databases/{database_name}/tables',
                'operation_id': 'create_table',
                'http_method': 'POST',
                'servers': None,
            },
            params_map={
                'all': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table1',
                ],
                'required': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table1',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'x_tenant':
                        (str,),
                    'layer_name':
                        (str,),
                    'database_name':
                        (str,),
                    'table1':
                        (Table1,),
                },
                'attribute_map': {
                    'x_tenant': 'X-Tenant',
                    'layer_name': 'layer_name',
                    'database_name': 'database_name',
                },
                'location_map': {
                    'x_tenant': 'header',
                    'layer_name': 'path',
                    'database_name': 'path',
                    'table1': 'body',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/vnd.graal.systems.v1.table+json'
                ],
                'content_type': [
                    'application/vnd.graal.systems.v1.table+json'
                ]
            },
            api_client=api_client
        )
        self.delete_table_by_id_endpoint = _Endpoint(
            settings={
                'response_type': None,
                'auth': [
                    'internal'
                ],
                'endpoint_path': '/layers/{layer_name}/databases/{database_name}/tables/{table_name}',
                'operation_id': 'delete_table_by_id',
                'http_method': 'DELETE',
                'servers': None,
            },
            params_map={
                'all': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table_name',
                ],
                'required': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table_name',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'x_tenant':
                        (str,),
                    'layer_name':
                        (str,),
                    'database_name':
                        (str,),
                    'table_name':
                        (str,),
                },
                'attribute_map': {
                    'x_tenant': 'X-Tenant',
                    'layer_name': 'layer_name',
                    'database_name': 'database_name',
                    'table_name': 'table_name',
                },
                'location_map': {
                    'x_tenant': 'header',
                    'layer_name': 'path',
                    'database_name': 'path',
                    'table_name': 'path',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/vnd.graal.systems.v1.error+json'
                ],
                'content_type': [],
            },
            api_client=api_client
        )
        self.find_table_by_id_endpoint = _Endpoint(
            settings={
                'response_type': (Table1,),
                'auth': [
                    'internal'
                ],
                'endpoint_path': '/layers/{layer_name}/databases/{database_name}/tables/{table_name}',
                'operation_id': 'find_table_by_id',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
                'all': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table_name',
                ],
                'required': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table_name',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'x_tenant':
                        (str,),
                    'layer_name':
                        (str,),
                    'database_name':
                        (str,),
                    'table_name':
                        (str,),
                },
                'attribute_map': {
                    'x_tenant': 'X-Tenant',
                    'layer_name': 'layer_name',
                    'database_name': 'database_name',
                    'table_name': 'table_name',
                },
                'location_map': {
                    'x_tenant': 'header',
                    'layer_name': 'path',
                    'database_name': 'path',
                    'table_name': 'path',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/vnd.graal.systems.v1.table+json',
                    'application/vnd.graal.systems.v1.error+json'
                ],
                'content_type': [],
            },
            api_client=api_client
        )
        self.find_tables1_endpoint = _Endpoint(
            settings={
                'response_type': ([Table1],),
                'auth': [
                    'internal'
                ],
                'endpoint_path': '/layers/{layer_name}/databases/{database_name}/tables',
                'operation_id': 'find_tables1',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
                'all': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                ],
                'required': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'x_tenant':
                        (str,),
                    'layer_name':
                        (str,),
                    'database_name':
                        (str,),
                },
                'attribute_map': {
                    'x_tenant': 'X-Tenant',
                    'layer_name': 'layer_name',
                    'database_name': 'database_name',
                },
                'location_map': {
                    'x_tenant': 'header',
                    'layer_name': 'path',
                    'database_name': 'path',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/vnd.graal.systems.v1.table+json'
                ],
                'content_type': [],
            },
            api_client=api_client
        )
        self.update_table_endpoint = _Endpoint(
            settings={
                'response_type': (Table1,),
                'auth': [
                    'internal'
                ],
                'endpoint_path': '/layers/{layer_name}/databases/{database_name}/tables/{table_name}',
                'operation_id': 'update_table',
                'http_method': 'PATCH',
                'servers': None,
            },
            params_map={
                'all': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table_name',
                    'patch',
                ],
                'required': [
                    'x_tenant',
                    'layer_name',
                    'database_name',
                    'table_name',
                    'patch',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'x_tenant':
                        (str,),
                    'layer_name':
                        (str,),
                    'database_name':
                        (str,),
                    'table_name':
                        (str,),
                    'patch':
                        ([Patch],),
                },
                'attribute_map': {
                    'x_tenant': 'X-Tenant',
                    'layer_name': 'layer_name',
                    'database_name': 'database_name',
                    'table_name': 'table_name',
                },
                'location_map': {
                    'x_tenant': 'header',
                    'layer_name': 'path',
                    'database_name': 'path',
                    'table_name': 'path',
                    'patch': 'body',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'application/vnd.graal.systems.v1.table+json',
                    'application/vnd.graal.systems.v1.error+json'
                ],
                'content_type': [
                    'application/json-patch+json;charset=UTF-8'
                ]
            },
            api_client=api_client
        )

    def create_table(
        self,
        x_tenant,
        layer_name,
        database_name,
        table1,
        **kwargs
    ):
        """Create a table  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.create_table(x_tenant, layer_name, database_name, table1, async_req=True)
        >>> result = thread.get()

        Args:
            x_tenant (str):
            layer_name (str): Name of the layer
            database_name (str): Name of the database
            table1 (Table1): The database to be created

        Keyword Args:
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
            async_req (bool): execute request asynchronously

        Returns:
            Table1
                If the method is called asynchronously, returns the request
                thread.
        """
        kwargs['async_req'] = kwargs.get(
            'async_req', False
        )
        kwargs['_return_http_data_only'] = kwargs.get(
            '_return_http_data_only', True
        )
        kwargs['_preload_content'] = kwargs.get(
            '_preload_content', True
        )
        kwargs['_request_timeout'] = kwargs.get(
            '_request_timeout', None
        )
        kwargs['_check_input_type'] = kwargs.get(
            '_check_input_type', True
        )
        kwargs['_check_return_type'] = kwargs.get(
            '_check_return_type', True
        )
        kwargs['_spec_property_naming'] = kwargs.get(
            '_spec_property_naming', False
        )
        kwargs['_content_type'] = kwargs.get(
            '_content_type')
        kwargs['_host_index'] = kwargs.get('_host_index')
        kwargs['x_tenant'] = \
            x_tenant
        kwargs['layer_name'] = \
            layer_name
        kwargs['database_name'] = \
            database_name
        kwargs['table1'] = \
            table1
        return self.create_table_endpoint.call_with_http_info(**kwargs)

    def delete_table_by_id(
        self,
        x_tenant,
        layer_name,
        database_name,
        table_name,
        **kwargs
    ):
        """Delete a table by an id  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.delete_table_by_id(x_tenant, layer_name, database_name, table_name, async_req=True)
        >>> result = thread.get()

        Args:
            x_tenant (str):
            layer_name (str): Id of the layer
            database_name (str): Id of the database
            table_name (str): Id of the table

        Keyword Args:
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
            async_req (bool): execute request asynchronously

        Returns:
            None
                If the method is called asynchronously, returns the request
                thread.
        """
        kwargs['async_req'] = kwargs.get(
            'async_req', False
        )
        kwargs['_return_http_data_only'] = kwargs.get(
            '_return_http_data_only', True
        )
        kwargs['_preload_content'] = kwargs.get(
            '_preload_content', True
        )
        kwargs['_request_timeout'] = kwargs.get(
            '_request_timeout', None
        )
        kwargs['_check_input_type'] = kwargs.get(
            '_check_input_type', True
        )
        kwargs['_check_return_type'] = kwargs.get(
            '_check_return_type', True
        )
        kwargs['_spec_property_naming'] = kwargs.get(
            '_spec_property_naming', False
        )
        kwargs['_content_type'] = kwargs.get(
            '_content_type')
        kwargs['_host_index'] = kwargs.get('_host_index')
        kwargs['x_tenant'] = \
            x_tenant
        kwargs['layer_name'] = \
            layer_name
        kwargs['database_name'] = \
            database_name
        kwargs['table_name'] = \
            table_name
        return self.delete_table_by_id_endpoint.call_with_http_info(**kwargs)

    def find_table_by_id(
        self,
        x_tenant,
        layer_name,
        database_name,
        table_name,
        **kwargs
    ):
        """Find table by Id  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.find_table_by_id(x_tenant, layer_name, database_name, table_name, async_req=True)
        >>> result = thread.get()

        Args:
            x_tenant (str):
            layer_name (str): Id of the layer
            database_name (str): Id of the database
            table_name (str): Id of the table

        Keyword Args:
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
            async_req (bool): execute request asynchronously

        Returns:
            Table1
                If the method is called asynchronously, returns the request
                thread.
        """
        kwargs['async_req'] = kwargs.get(
            'async_req', False
        )
        kwargs['_return_http_data_only'] = kwargs.get(
            '_return_http_data_only', True
        )
        kwargs['_preload_content'] = kwargs.get(
            '_preload_content', True
        )
        kwargs['_request_timeout'] = kwargs.get(
            '_request_timeout', None
        )
        kwargs['_check_input_type'] = kwargs.get(
            '_check_input_type', True
        )
        kwargs['_check_return_type'] = kwargs.get(
            '_check_return_type', True
        )
        kwargs['_spec_property_naming'] = kwargs.get(
            '_spec_property_naming', False
        )
        kwargs['_content_type'] = kwargs.get(
            '_content_type')
        kwargs['_host_index'] = kwargs.get('_host_index')
        kwargs['x_tenant'] = \
            x_tenant
        kwargs['layer_name'] = \
            layer_name
        kwargs['database_name'] = \
            database_name
        kwargs['table_name'] = \
            table_name
        return self.find_table_by_id_endpoint.call_with_http_info(**kwargs)

    def find_tables1(
        self,
        x_tenant,
        layer_name,
        database_name,
        **kwargs
    ):
        """Retrieve all tables  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.find_tables1(x_tenant, layer_name, database_name, async_req=True)
        >>> result = thread.get()

        Args:
            x_tenant (str):
            layer_name (str): Name of the layer
            database_name (str): Name of the database

        Keyword Args:
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
            async_req (bool): execute request asynchronously

        Returns:
            [Table1]
                If the method is called asynchronously, returns the request
                thread.
        """
        kwargs['async_req'] = kwargs.get(
            'async_req', False
        )
        kwargs['_return_http_data_only'] = kwargs.get(
            '_return_http_data_only', True
        )
        kwargs['_preload_content'] = kwargs.get(
            '_preload_content', True
        )
        kwargs['_request_timeout'] = kwargs.get(
            '_request_timeout', None
        )
        kwargs['_check_input_type'] = kwargs.get(
            '_check_input_type', True
        )
        kwargs['_check_return_type'] = kwargs.get(
            '_check_return_type', True
        )
        kwargs['_spec_property_naming'] = kwargs.get(
            '_spec_property_naming', False
        )
        kwargs['_content_type'] = kwargs.get(
            '_content_type')
        kwargs['_host_index'] = kwargs.get('_host_index')
        kwargs['x_tenant'] = \
            x_tenant
        kwargs['layer_name'] = \
            layer_name
        kwargs['database_name'] = \
            database_name
        return self.find_tables1_endpoint.call_with_http_info(**kwargs)

    def update_table(
        self,
        x_tenant,
        layer_name,
        database_name,
        table_name,
        patch,
        **kwargs
    ):
        """Update a table  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True

        >>> thread = api.update_table(x_tenant, layer_name, database_name, table_name, patch, async_req=True)
        >>> result = thread.get()

        Args:
            x_tenant (str):
            layer_name (str): Id of the layer
            database_name (str): Id of the database
            table_name (str): Id of the table
            patch ([Patch]): The patch

        Keyword Args:
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
            async_req (bool): execute request asynchronously

        Returns:
            Table1
                If the method is called asynchronously, returns the request
                thread.
        """
        kwargs['async_req'] = kwargs.get(
            'async_req', False
        )
        kwargs['_return_http_data_only'] = kwargs.get(
            '_return_http_data_only', True
        )
        kwargs['_preload_content'] = kwargs.get(
            '_preload_content', True
        )
        kwargs['_request_timeout'] = kwargs.get(
            '_request_timeout', None
        )
        kwargs['_check_input_type'] = kwargs.get(
            '_check_input_type', True
        )
        kwargs['_check_return_type'] = kwargs.get(
            '_check_return_type', True
        )
        kwargs['_spec_property_naming'] = kwargs.get(
            '_spec_property_naming', False
        )
        kwargs['_content_type'] = kwargs.get(
            '_content_type')
        kwargs['_host_index'] = kwargs.get('_host_index')
        kwargs['x_tenant'] = \
            x_tenant
        kwargs['layer_name'] = \
            layer_name
        kwargs['database_name'] = \
            database_name
        kwargs['table_name'] = \
            table_name
        kwargs['patch'] = \
            patch
        return self.update_table_endpoint.call_with_http_info(**kwargs)

